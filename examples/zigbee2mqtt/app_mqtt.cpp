...
void appMQTTStart(void) {
    char uri[128] = {0};
    sprintf(uri, "mqtt://%s:%d", mqttServer.c_str(), mqttPort);
    esp_mqtt_client_config_t mqtt_cfg = {
        .uri = uri,
        // .host = mqttServer.c_str(),
        // .port = mqttPort
    };
    if (mqttUsername != "") {
...
static void msg_handler(
    ...
    for (size_t i = 0; i < 10; i++) {
        if (subTopics[i].topic != NULL && subTopics[i].handle != NULL) {
            // printf("topic: %s\n", subTopics[i].topic);
            if (!memcmp(subTopics[i].topic, topic_new, strlen(subTopics[i].topic))) {
                subTopics[i].handle(topic_new, data_new);
            }
        }
    }
...
static void topic_init(void) {
    for (size_t i = 0; i < 10; i++) {
        subTopics[i].topic = NULL;
        subTopics[i].handle = NULL;
    }
}
